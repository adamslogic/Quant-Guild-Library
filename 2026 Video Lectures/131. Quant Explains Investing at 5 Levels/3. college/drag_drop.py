"""College pygame drag-and-drop scene.

- Background: `nyse.png` (New York Stock Exchange floor).
- M and I each toggle a separate money image.
- S toggles the red stick guy and the ninja guy together.
- Every other image in the folder is a normal draggable prop.

Controls:
  - M                 : show / hide money image #1
  - I                 : show / hide money image #2
  - S                 : show / hide the red stick guy + the ninja guy
...
RED_GUY_SPRITE = "stick_guy_red.png"
NINJA_SPRITE = "ninja_guy.jpg"

# Files handled specially (not treated as generic props).
SPECIAL_FILES = {BACKGROUND, MONEY_SPRITE, RED_GUY_SPRITE, NINJA_SPRITE}
...
def main():
    pygame.init()
    pygame.display.set_caption("College - NYSE Drag & Drop")
    ...
    # Two independent money images toggled with M and I.
    money_image = load_image(MONEY_SPRITE, MAX_SPRITE_SIZE)
    money1 = Draggable(money_image, (0, 0))
    money2 = Draggable(money_image, (0, 0))
    show_money1 = True
    show_money2 = True

    # Red stick guy + ninja guy toggle together with S.
    red_guy = Draggable(load_image(RED_GUY_SPRITE, MAX_SPRITE_SIZE), (0, 0))
    ninja = Draggable(load_image(NINJA_SPRITE, MAX_SPRITE_SIZE), (0, 0))
    show_s_group = True

    toggled = [money1, money2, red_guy, ninja]
    ...
    random_positions(props + toggled, screen_size)

    def active_sprites():
        sprites = list(props)
        if show_money1:
            sprites.append(money1)
        if show_money2:
            sprites.append(money2)
        if show_s_group:
            sprites.append(red_guy)
            sprites.append(ninja)
        return sprites
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    show_money1 = not show_money1
                    if not show_money1:
                        money1.stop_drag()
                elif event.key == pygame.K_i:
                    show_money2 = not show_money2
                    if not show_money2:
                        money2.stop_drag()
                elif event.key == pygame.K_s:
                    show_s_group = not show_s_group
                    if not show_s_group:
                        red_guy.stop_drag()
                        ninja.stop_drag()
